...
onLoadName = "ENDGAME ALTIS";           //mission name (using allcaps looks better)
...
VEHICLERESPAWNTIME = 600;               //time after which vehicles respawn
...
DLICONRANGE = -1;                       //range in meters at which 3D icon on intel is displayed - -1 to disable entirely - WARNING: values lower than 4 may cause inability to access large download objects (vehicles, cargo containers, ...)
...
MAXTIMETOENDGAME = 6600;                //maximum time in seconds, after which phase 3 will start regardless of how much intel was found - set to -1 to disable
