	...
	while {zbe_RUN} do {
		...
		zbe_players = (switchableUnits + playableUnits);
		{
			...
			if (!_disable && !(_x in zbe_cachedGroups)) then {
				 ...
				 _fsmHandle = [zbe_aiCacheDist, _x, zbe_minFrameRate, zbe_debug] execFSM "zbe_cache\zbe_aiCaching.fsm";
				 zbe_AIFSMs pushBack _fsmHandle;
				};
		} forEach allGroups;
	};
};
		...
		_assetscar = zbe_centerPOS nearEntities ["LandVehicle", zbe_mapside];
		{
			if !(_x in zbe_cached_cars) then {
				zbe_cached_cars = zbe_cached_cars + [_x];
					_fsmHandle = [_x, zbe_vehicleCacheDistCar] execFSM "zbe_cache\zbe_vehicleCaching.fsm";
					zbe_VehicleFSMs pushBack _fsmHandle;
			};
		} forEach _assetscar;
		_assetsair = zbe_centerPOS nearEntities ["Air", zbe_mapside];
		{
			if !(_x in zbe_cached_air) then {
				zbe_cached_air = zbe_cached_air + [_x];
				    _fsmHandle = [_x, zbe_vehicleCacheDistAir] execFSM "zbe_cache\zbe_vehicleCaching.fsm";
					zbe_VehicleFSMs pushBack _fsmHandle;
			};
		} forEach _assetsair;
		_assetsboat = zbe_centerPOS nearEntities ["Ship", zbe_mapside];
		{
			if !(_x in zbe_cached_boat) then {
				zbe_cached_boat = zbe_cached_boat + [_x];
					_fsmHandle = [_x, zbe_vehicleCacheDistBoat] execFSM "zbe_cache\zbe_vehicleCaching.fsm";
					zbe_VehicleFSMs pushBack _fsmHandle;
			};
		} forEach _assetsboat;
